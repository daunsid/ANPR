...
from core.base import BaseDetector, BaseOCR, DetectionResult, OcrResult
from fast_alpr.default_ocr import DefaultOCR
from src.detector import Yolov11ObjectDetector
...
class ALPR:
    ...
    def __init__(
        ...
        self.detector = detector or Yolov11ObjectDetector(
    ...
    def predict(self, frame: np.ndarray | str) -> list[ALPRResult]:
        ...
        detection_output = self.detector.predict([frame])
        alpr_results = []
        for plate_detections in detection_output:
            for detection in plate_detections:
                bbox = detection.bounding_box
                x1, y1 = max(bbox.x1, 0), max(bbox.y1, 0)
                x2, y2 = min(bbox.x2, frame.shape[1]), min(bbox.y2, frame.shape[0])
                cropped_plate = frame[y1:y2, x1:x2]
                # Resize the cropped image (e.g., scale up by 2x)
                # upscale_factor = 2.0
                # resized_plate = cv2.resize(cropped_plate, (0, 0), fx=upscale_factor, fy=upscale_factor, interpolation=cv2.INTER_CUBIC)
